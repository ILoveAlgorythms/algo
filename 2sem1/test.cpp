...
#include <algorithm>
// template <class T>
using FilterFunction = std::function<bool()>;
...
class ListGraph : public Graph<VType, EType> {
  public:
    ...
    IteratorImpl<VType, EType> NeighboursIt(
      VType v, 
      const FilterFunction& filter = []() { return true; }
    ) override {
...
void DFS(
  ...
  colors[from] = grey;
  for (auto edge : graph.NeighboursIt(from, [&colors](){return false;})) {
...
int main() {
  ...
  for (auto edge : g.NeighboursIt(3)) {
